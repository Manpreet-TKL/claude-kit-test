...
**How to apply:** Reset such passwords through the model pipeline mimicking the command: load the `UserAuthentication`, set `password`/`password_repeat` (+ `password_status` CURRENT, zero `password_failed_tries`), `validate()`, ignore *only* the `institution_authentication_id` error, then `save(false)` - bcrypt hashing happens in `afterValidate()`. Write the audit row inside try/catch with a short message. Run scripts with `docker exec -i <container> php < file` (container /tmp is tmpfs; `docker cp` writes are shadowed). Related: [[monkey-environment]].

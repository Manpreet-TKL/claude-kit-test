...
description: "GNU screen 5 gotchas - defbce changes TERM to -bce breaking colour prompts, emoji VS16 gets mangled, detached tests show TERM=unknown, Claude Code copy leaks OSC 52 base64"
metadata: 
  node_type: memory
  type: project
  originSessionId: 94beccc2-8787-4967-8238-2ce6947be6d5
  modified: 2026-07-19T10:24:36.630Z
---

Found 2026-07-12 while debugging "white prompt + _ _ line" inside the source-built screen 5.0.1 (installed by `~/claude-kit/scripts/screen5_install.sh`):

1. `defbce on` makes screen advertise `TERM=screen-256color-bce`. The stock Ubuntu ~/.bashrc colour case only matches `xterm-color|*-256color`, so the prompt (and colour detection generally) goes white. Fix: never add defbce; screen5_install.sh no longer does.
...
5. Copying in Claude Code inside screen dumps a base64 blob into the prompt instead of setting the Windows clipboard (seen 2026-07-19). Claude sets the clipboard via OSC 52 (`ESC]52;c;<base64>BEL`); screen breaks the escape wrapper on long OSC strings so the payload leaks as printable base64. screen has no OSC 52 passthrough option (tmux does: set-clipboard on). Workarounds: copy outside screen (detach / second SSH window), or decode a leaked blob with `echo 'BLOB' | base64 -d`; when handing text to the user for copying, Write it to a file instead of printing it.

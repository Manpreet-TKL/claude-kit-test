...
**Applies to claude-kit:** `install.sh` registers atlassian/github/codex via `claude mcp add-json <name> "$json" -s user` (teardown: `claude mcp remove <name> -s user`) - the earlier jq-merge into `settings.json` was the bug this memory caught. The atlassian server needs a classic (unscoped) API token - `docs/atlassian.md` in the kit covers scoped-token 401s and the gateway/cloudId lookup.

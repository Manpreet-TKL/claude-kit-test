...
Two transports for both demographics and appointments: **HL7 v2.x** (Appendix A) and the **native OpenEyes RESTful PAS API** (Appendix B). Connect remaps all HL7 to the PAS API, so the PAS API bounds what is transferable. See `c-pasapi` for PASAPI v2 detail and `c-mcchannels` for the channels.

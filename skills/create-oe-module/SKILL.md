...
3. Write `config/common.php` (params, components, sub-modules as needed).
4. Add the explicit-class entry to `$modules` in `protected/config/core/common.php` (or `local/common.php` for testing).
5. If you need a top-nav entry: add `params.menu_bar_items` AND set `params.oe_special_module = true` — but only if this is an admin/infra tool. Clinical modules don't.
6. Write a timestamped migration in `protected/modules/OphXxFoo/migrations/` that creates tables, inserts the `event_type` and `element_type` rows (event-type modules), and any `authitem` permissions.
7. Clear APCu: `curl http://localhost/apc_clear.php`.
8. Run `./protected/yiic migrate --all`.
9. Add a `<Name>_API.php` in `components/` if other modules need to read from yours.
10. PHPStan/PHPCS the new files against the **yii** configs (`phpstan.yii.neon`, `phpcs.yii.xml`).

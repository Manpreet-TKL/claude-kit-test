...
3. In chat, print the file path, the headline `M tickets -> unique_fix_count unique fixes`, and a ranked one-line list of clusters with counts and the automatable flag (`x7 reindex-worklist (automatable) ... x3 restart-mirth ...`) so the automation candidates are obvious at a glance.

...
**Fan out:** divide the full list into batches of **20**. State the agent count
and ask once for cost confirmation, then use the current client's native
collaboration facility to run one self-contained subagent per batch in
parallel. Use the current or inherited model unless the user explicitly
approves an override. Each agent gets this prompt (`[BATCH]` = that batch's
ticket data):

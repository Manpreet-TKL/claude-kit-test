...
Subs: `subs/paths.md` - Read only the slice you need (offset/limit); section index, one heading per line (re-anchor with `grep -n '^#' subs/paths.md` if edits drift it): login L5, global chrome L12, main menu L19, patient search->summary L54, Add Event dialog + event-type table L61, Document create form, PDF upload + page previews L100, event view/edit/delete L111, worklist L118, admin L129, ends L137. `subs/examination.md` (Examination event + element manager + element census pointers), `subs/probe.md` (in-container Puppeteer probe: discovery, driver + full action schema, endpoints-for-proof, subagent template, policies). Load `subs/probe-playwright.md` only for images without the bundled Puppeteer/Chrome (dev/debug images carry Playwright; remote-chrome stacks) or when a Playwright run is explicitly requested. Drivers: `scripts/journey.mjs` (Puppeteer, primary), `scripts/journey.playwright.mjs` (fallback).

#!/usr/bin/env node
// OpenEyes journey driver: run a JSON action list against a running OE instance
// and print a compact text dump of what the user sees after every step.
// Usage + container invocation: ../subs/probe.md
//
//   node journey.mjs <actions.json | ->  [--shot <dir>]
//
...
//
// Env: BASE_URL (http://web) · OE_USERNAME/OE_PASSWORD (admin/admin) or
...
const BASE = env('BASE_URL', 'http://web').replace(/\/$/, '');
...
let actions;
try {
  actions = JSON.parse(readFileSync(argv[0] && argv[0] !== '-' ? argv[0] : 0, 'utf8'));

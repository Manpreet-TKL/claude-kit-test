...
- **Need to watch every step** (debugging a flow that fails at an unknown point)? Add `--verbose` (or `-e OE_VERBOSE=1`) to restore a full dump after every step - a 3-step walk is ~3x the output, longer walks more, so reach for it only while diagnosing.

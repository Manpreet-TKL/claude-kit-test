...
**Getting onto the form (2026-08-05, develop):** `/patientEvent/create?...&event_type_id=32` does **not** open this form - it opens a 'Please select booking' chooser ("Create Consent Form") listing bookings with and without consent forms plus a 'Create New Consent for Unbooked Procedure(s)' block. The form itself is at `/OphTrConsent/Default/create?patient_id=<pid>&unbooked=1`, which is where the Standard form row's 'Create consent' button goes; template rows post an eye (R/L) and a template id instead. `/update/<event_id>` opens the same form on a saved event.

**Type 4 differs by more than field visibility (2026-08-05, develop):** '4. Unable to consent' replaces most of the agreement elements with a capacity/best-interest pathway - element type ids 522 Capacity Assessment, 523 Best Interest Decision, 525 Medical Capacity Advocate, 526 Patient Attorney/Deputy, 530 Others Involved, plus 518 Special requirements and 529 Consent Taken by. Radio/checkbox groups in these elements are reached by their **label**, not by name, and two markups are in use: the input inside its own `<label>`, and Yii `checkBoxList`, which emits `<input id=X>` with a sibling `<label for=X>` - a locator assuming the wrapping form silently finds nothing on the second.

**Confirm and withdraw are navigations, not dialogs (2026-08-05, develop):** on the event **view**, `.js-add-confirm` and `.js-add-withdrawal` are plain `window.location.href` jumps to `/OphTrConsent/default/{confirm,withdraw}?event_id=<id>` (`assets/js/module.js:404-412`); the controller mutates the event on that GET and redirects back to the view, so the record has already changed before anything is signed. On a Type 4 the withdrawal is then finished in the **E-Sign** element, not in Withdrawal (532), which shows only a sentence pointing there: `#add_patient_contact_button` (labelled 'Add withdrawal') opens an adder for the signatory, and only once one is recorded does the reason box appear - `textarea[placeholder="Withdrawal reason (required)"]` (`#Element_OphTrConsent_Esign_withdrawal_reason`) with `#withdraw-submit-button`, 'Proceed with withdraw consent'.

...
Subs: `subs/protected-tree.md` (per-dir map + era key), `subs/event-element-model.md` (Patient→Episode→Event→Element spine, base classes, view triad, `et_` naming), `subs/cli-jobs.md` (yiic/artisan, the job-dispatch bridge, migrations/factories/seeders), `subs/clinical-modules.md` (module catalogue), `subs/examination-elements.md` (OphCiExamination element types), `subs/explore.md` (goal-keyed jump table).

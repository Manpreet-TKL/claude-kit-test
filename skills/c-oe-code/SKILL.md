...
## Yii 1 AR + menu-cache pitfalls

Eager-loading (`with()`) a HAS_MANY/MANY_MANY relation whose definition carries a `condition` (e.g. `is_active = 1`) puts that condition in the joined query's outer WHERE and silently **drops primary rows** that have no matching related row - lazy loading applies it to related rows only, so the two paths return different primary sets and no query count or timing shows it. Fix at query time: `'relname' => ['together' => false]` in the `with()` array (separate grouped query, lazy-equivalent semantics); BELONGS_TO eager joins never filter primary rows. When converting lazy loads to eager, verify by normalized HTML diff of the rendered page, not query deltas. Separately, `protected/components/MenuHelper.php` `getMenu()` caches the built main menu for 3600s per session id + institution + patient (`invalidateCache()` bumps a namespace key rather than deleting): fresh logins and the first view of each patient pay the full menu build (~230 lazy queries in a many-queue-set institution via PatientTicketing `getMenuItems`), warm sessions never do - measuring menu cost needs a fresh login per run, and a warm-session benchmark masks any menu regression or improvement.

...
The server runs the host `codex` binary as `codex mcp-server` and is registered with
its defaults baked in (set by `install.sh -x`): the **flagship model at high reasoning
effort**, in a **workspace-write, network-off sandbox**, `approval_policy=never` so
agents run unattended. It exposes two tools:
...
2. **Signed in?** Codex auth is the host's ChatGPT login. Run `codex login status`
   (read-only). If it reports not logged in, tell the user to run `codex login` on the
   host and stop — agent calls will fail until then.
...
- **Not signed in / 401** → run `codex login` on the host, then stop.
...
- **No network** (`workspace-write` + `network_access=false`) → an agent **cannot
  `git push`**, fetch, or call out. This is the sandbox analogue of the never-push floor.
- **Never commit.** `git commit` is local, so the sandbox can't block it — **instruct
  every agent not to commit or stage**; the human commits (see `create-oe-pr`). Reject
  any agent output that committed.
- **Workspace-only writes.** Agents edit within their `cwd` tree; they can't write
  outside it. Don't point an agent at `~` or `/`.
...
command -v codex >/dev/null 2>&1 \
  || echo "codex CLI not installed — npm install -g @openai/codex  (or: brew install codex)"
codex login status >/dev/null 2>&1 \
  || echo "not signed in — run 'codex login' on this host (ChatGPT sign-in)"
...
out of band into this session. Once the binary is installed, you're signed in, and the
...
- `CODEX_SANDBOX` — default `workspace-write` (network off).

...
This repo only produces the **image**. Actual deployment lives in `oe-deploy` (sibling `octopus/` worktree), which assembles `templates/notes.yml` + `templates/db.yml` + `templates/tfk_only.yml` into a `docker-compose.yml` via `build.sh`. For deployment work, switch to the `oe-deploy` skill.

...
- The Steps/Current/Expected triad lives in the `# JIRA` description box and fits Bug/Regression only - drop it for feature/planning/performance types; skip lighter sections when self-evident; never pad or fabricate steps. For the Steps to Reproduce, run `a-oe-repro` if they aren't already in this conversation - it owns the rules. It returns up to two blockquotes (an optional `**Environment setup**` preamble, then the steps); a missing setup block is the normal case. **Its evidence files - log slices, exception JSON, screenshots, GIFs - stay out of the PR folder**: they live in `~/repro-evidence/<date>-<slug>/` and are attached to the Jira ticket by you. The folder carries `PR.md` and the patch, nothing else.
...
- **The prose must read as hand-written code**, with no trace of how the change was delivered: no *patch* / *hunk* / *apply* / *rebase* / *conflict* anywhere in the Jira or GitHub blocks (only the `## Apply & push` command list may mention them), never the branch(es) it was built or tested on (the PR page shows the merger the base and head already), and never how a request is served (say "the request" / "request-scoped" - never the web server, SAPI or worker model; **OpenEyes runs Apache + `mod_php`, never PHP-FPM**). See `subs/reference.md` -> *Prose rules*.

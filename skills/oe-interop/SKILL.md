---
name: oe-interop
description: OpenEyes third-party integration capabilities and how to configure each
...
- Siblings: `mcc` (BridgeLink/Mirth engine + REST), `mcchannels` (the actual OE PAS/interop channels), `pasapi` (PASAPI v2 + OpenMRS→BridgeLink→OE flow), `oe-iolmaster-import` and `oe-payload-processor` (DICOM/device-result importers feeding device events).

...
Generic connector mechanics and the Velocity/auth gotcha: the `mcc` skill. This
...
- **Destinations:** 6 × HTTP Sender, all `PUT`/`DELETE` to `http://web/PASAPI/...`
...
there goes out literally and 401s — see `mcc`). So for the PAS IN and DICOM HTTP

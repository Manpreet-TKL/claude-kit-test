---
name: mcchannels
description: The three OE integration channels (PAS IN/OUT, DICOM) in BridgeLink
...
REST recipes, lifecycle, and gotchas: the `mcc` skill. OE PASAPI endpoint detail
and the PAS flow: the `pasapi` skill. This skill covers what each *channel* does.

## The channels

| Channel | Dir | Source connector (port) | Destinations | OE endpoint(s) |
|---|---|---|---|---|
| PAS IN | inbound | TCP MLLP Listener `:6661` | 6 × HTTP Sender | `PUT http://web/PASAPI/...` |
...
`username`/`password`). See `subs/secrets-config.md` and the `mcc` skill.
...
import. `LOCAL-1-0` is also hardcoded throughout (5× in PAS IN, 3× in DICOM)

...
applyCodex() {
    ...
    if [ "${cx_runtime}" = "docker" ]; then
        ...
        # the docker "--name + rm -f" reuse. We match the "codex mcp-server"
        # cmdline and skip our own shell via $$; matching on "mcp-server" leaves
        # an interactive `codex` untouched. Each arg is @sh-quoted so the shell
        # hands codex the exact same argv (TOML string quotes intact).
        run_cmd="$(jq -rn --arg gate "${gate}" --argjson a "${args_json}" \
            '$gate + "for p in $(pgrep -f \"codex mcp-server\"); do [ \"$p\" != \"$$\" ] && kill \"$p\" 2>/dev/null; done; exec codex " + ($a | map(@sh) | join(" "))')"
    fi
